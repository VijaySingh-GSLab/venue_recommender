...
#@st.cache()
@st.cache(suppress_st_warning=True)
def plot_count_graph(graph_count):
...
def main():
    ...
    # map1
    plot_df_map_nbhd = X_dest_raw[X_dest_raw[col_grain].isin(X_match_sorted_named[col_grain])].copy()
    plot_df_map_nbhd = plot_df_map_nbhd.drop_duplicates(subset=[col_grain])
    # use mean values of lat/long (Imppp)
    map_graph_nbhd = plot_nbhd_on_map(plot_df=plot_df_map_nbhd, marker_size=20, map_zoom=11, show_plot=False)
    st.plotly_chart(map_graph_nbhd)

    

    list_dest_venues = X_match_sorted_named[col_grain].values
    selected_nbhd = st.selectbox('Select the most suitable neighborhood:', list_dest_venues)
    #with st.spinner('Generating the map of venues present in {} ....'.format(selected_nbhd)):
    """

    """
    if st.button("Plot Map"):
        plot_df_map_venue = X_dest_raw[X_dest_raw[col_grain] == selected_nbhd].copy()
        map_graph_venue = plot_nbhd_on_map(plot_df=plot_df_map_venue, marker_size=18, map_zoom=13, show_plot=False, col_text=col_feature_name)
        st.plotly_chart(map_graph_venue)
    """
    print('Done')
